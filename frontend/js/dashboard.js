...
let svg = parent.append("svg").attr("width", boxSize).attr("height", boxSize);

let g = svg
  .append("g")
  .attr("transform", "translate(" + boxSize / 2 + "," + boxSize / 2 + ")");

let meter = g.append("g").attr("class", "meter");
...
function updateProgress(progress) {
  ...
  nbCalories.text(
    parseInt(progress * objective) + " / " + objective + " Calories"
  );
}
  ...
  $.ajax({
    ...
    .done(function (data) {
      for (let i in data["data"]) {
        $("#inputRepasName").append(
          `<option value="${data["data"][i]["nom"]}"> ${data["data"][i]["nom"]} </option>`
        );
      }
    })
    .fail(function () {
...
function journalRepasDashboard() {
  $.ajax({
    ...
    .done(function (data) {
      ...
      console.log(data);
      for (let i in data) {
        $("#journalRepasDashboard table tbody").append(
          `<tr><td class="table-date">${data[i]["date"]}</td><td class="table-item-name">${data[i]["nom"]}</td><td class="table-quantity">${data[i]["quantite"]}</td><td class="table-calories">${data[i]["aliment.nb_calories*repas.quantite"]}</td></tr>`
        );
      }
    })
    .fail(function () {
...
function journalSportDashboard() {
  $.ajax({
    ...
    .done(function (data) {
      ...
      console.log(data);
      for (let i in data) {
        $("#journalSportDashboard table tbody").append(
          `<tr><td class="table-date">${data[i]["date"]}</td><td class="table-item-name">${data[i]["nom"]}</td><td class="table-duration">${data[i]["temps"]} min</td><td class="table-calories">${data[i]["sport.nb_calories*pratique.temps/60"]}</td></tr>`
        );
      }
    })
    .fail(function () {
